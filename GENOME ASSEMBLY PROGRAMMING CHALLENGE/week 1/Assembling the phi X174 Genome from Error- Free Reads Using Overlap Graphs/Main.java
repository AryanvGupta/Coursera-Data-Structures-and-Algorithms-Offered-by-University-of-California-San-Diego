...
class Main{
    public static void main(String[] args) throws IOException {
        ...
        List<GenomeFromErrorFreeReads.Read> reads = new ArrayList<GenomeFromErrorFreeReads.Read>();
        String line;
        int index = 0;
        while ((line = reader.readLine()) != null) {
            if (!line.isEmpty()) {
                reads.add(new GenomeFromErrorFreeReads.Read(index++, line));
            } else {
                break;
            }
        }
        new Thread(null, new Runnable() {
            public void run() {
                try {
                    GenomeFromErrorFreeReads.OutputWriter writer = new GenomeFromErrorFreeReads.OutputWriter(System.out);
...
class GenomeFromErrorFreeReads {
