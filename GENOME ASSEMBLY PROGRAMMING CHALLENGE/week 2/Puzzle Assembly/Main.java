...
public class Main {
    public static void main(String[] args) throws IOException {
        PuzzleAssembly.FastReader reader = new PuzzleAssembly.FastReader();
        List<PuzzleAssembly.PuzzlePiece> puzzlePieces = new ArrayList<PuzzleAssembly.PuzzlePiece>();
        for (int i = 1; i <= 25; i++) {
            String line = reader.nextLine();
            puzzlePieces.add(new PuzzleAssembly.PuzzlePiece(line.trim().replace("(", "").replace(")", "").split(",")));
        }

        PuzzleAssembly.PuzzlePiece[][] solution = PuzzleAssembly.solve(puzzlePieces);

        try {
            for (int row = 0; row < 5; row++) {
                StringBuilder line = new StringBuilder();
                for (int col = 0; col < 5; col++) {
                    line.append(PuzzleAssembly.generatePieceString(solution[row][col]) + (col == 4 ? "" : ";"));
                }
...
class PuzzleAssembly {
    ...
    protected static String generatePieceString(PuzzlePiece puzzlePiece) {
    ...
    protected static PuzzlePiece[][] solve(List<PuzzlePiece> pieces) {
    ...
    protected static class PuzzlePiece {
